  ...
  def main(args: Array[String]) {
    ...
    queries.foreach(q => {
      ...
      if(q.slotsToFill.size == 1){
        ...
        outStatsStream.println(q.name)
        outStatsStream.println("numAnswers: " + numAnswers + " " + slotToFill.name)
        outStatsStream.println("o i m size: " + openieList.size + " " + implieList.size + 
          " " + multirList.size) 
